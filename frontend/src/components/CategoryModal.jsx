...
const CategoryModal = ({ isOpen, onClose, onSave, categoryToEdit = null }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-slate-800 rounded-lg shadow-2xl border border-slate-700 p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="📚"
              maxLength="2"
              className="w-full bg-slate-700/50 border border-slate-600 rounded-lg px-3 py-2 text-white text-2xl focus:outline-none focus:border-cyan-500 transition-colors"
            />
            <p className="text-xs text-gray-500 mt-1">Tip: Copy an emoji from emojipedia.org</p>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-3">
              <input
                ...
                onChange={handleChange}
                className="h-10 w-20 bg-slate-700 border border-slate-600 rounded cursor-pointer"
              />
              <input
                type="text"
                value={formData.color}
                ...
                placeholder="#3B82F6"
                className="flex-1 bg-slate-700/50 border border-slate-600 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-cyan-500 transition-colors"
              />
            </div>
          </div>
